// AI-laag: OpenAI als er een OPENAI_API_KEY is, anders Claude als er een ANTHROPIC_API_KEY is, anders de offline engine.
import OpenAI from "openai";
import Anthropic from "@anthropic-ai/sdk";
import * as engine from "./engine.js";
import { SIGNALS, PILLARS, TEST_QUESTIONS, COMPLIMENTS, NOTICED, SCENARIOS } from "./knowledge.js";

const provider = process.env.OPENAI_API_KEY ? "openai" : process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_AUTH_TOKEN ? "claude" : "engine";
const MODEL = process.env.CAPTAIN_MODEL || (provider === "openai" ? "gpt-4.1" : "claude-opus-5");
const openai = provider === "openai" ? new OpenAI() : null;
const anthropic = provider === "claude" ? new Anthropic() : null;
const client = openai || anthropic;

export const aiEnabled = provider !== "engine";
export const aiProvider = provider;
export const aiModel = aiEnabled ? MODEL : null;
...
async function askJSON(system, user, maxTokens = 1800) {
  ...
  if (openai) {
    ...
    if (!choice || choice.finish_reason === "content_filter") throw new Error("refusal");
  ...
  try {
    ...
    if (err?.status === 400) response = await anthropic.messages.create(params);
    else throw err;
  }
  if (response.stop_reason === "refusal") throw new Error("refusal");
  return parseJSON(response.content.filter((b) => b.type === "text").map((b) => b.text).join(""));
}
...
{"comfort": 0-100 (hoe veilig/open haar zenuwstelsel lijkt), "state": "sympathisch"|"overgang"|"parasympathisch", "stateLabel": korte NL beschrijving,
 ...
 "negatives": [{"label": korte NL tekst}], "interpretation": 2-4 zinnen NL, "move": 1-2 zinnen concrete kapiteinszet, beginnend met "Kapiteinszet:"}`,
    ...
    return { ...base, ...out, signals, source: "ai" };
  } catch (err) {
...
{"pressure": 0-100 (hoeveel onrust/druk het origineel uitstraalt), "diagnosis": [{"label": kort NL, "detail": 1 zin NL}],
 ...
 "groundedExit": {"text": korte NL tekst voor de optie niets sturen, "why": 1-2 zinnen}}`,
    ...
    return { ...base, ...out, pressure, frame: 100 - pressure, source: "ai" };
  } catch (err) {
...
{"reply": tekst van ${scenario.persona}, "analytics": {"frame": 0-100, "anxiety": "Laag / veilig"|"Verhoogd"|"Hoog", "flags": [korte NL labels]}, "note": 1-2 zinnen coachnotitie, direct en concreet}`,
    ...
    return { ...base, ...out, persona: scenario.persona, source: "ai" };
  } catch (err) {
